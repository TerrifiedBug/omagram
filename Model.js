...
// OmaGram's own scheme: time today, "Yesterday", weekday this week, date beyond.
...
function chatFallbackName(chat) {
  if (!chat) return ""
...
// Telegram status enum: 1 pending, 2 sent, 4 read.
// Two nf-fa-check glyphs keep the read state legible across Nerd Fonts.
...
function formatMessageText(text, linkColor) {
  ...
  // OmaGram document attachments start with \uf15c icon. Protect document filename header.
  var docPrefix = ""
