...
// The OmaGram surface: chat list, conversation, inline reply, and the QR
...
Panel {
  ...
  readonly property bool needsApi: client ? client.needsApi === true : false
  ...
  readonly property bool linked: client ? client.linked === true : false
  readonly property bool showLogin: needsApi || needsLogin
  ...
  function activateCursor() {
    ...
    if (chat) root.selectChat(chat.chatId)
  }

  function sendReply() {
    var text = composer.text
    if (!text || !text.trim().length) return
    if (!root.client || !root.linked || root.client.connectionState !== "open") {
      root.statusLine = "Not connected to Telegram"
      return
    }
    if (root.client.sendText(root.activeChatId, text)) {
      composer.text = ""
      root.statusLine = ""
      typingTimer.stop()
      root.client.setTyping(root.activeChatId, "paused")
    }
  }

  function submitPassword() {
    if (!root.client || passwordField.text.length === 0) return
    root.client.sendPassword(passwordField.text)
    passwordField.text = ""
  }
  ...
  function refreshChats() {
    ...
    if (!root.linked || root.client.connectionState !== "open") {
      root.statusLine = "Not connected to Telegram"
      return
    }
    ...
    root.client.refresh()
    if (root.view === "chat" && root.activeChatId)
      root.client.openChat(root.activeChatId)
  ...
  onOpenedChanged: {
    ...
    if (root.client) {
      ...
      if (root.view === "chat" && root.activeChatId) {
        root.client.openChat(root.activeChatId)
        root.client.markRead(root.activeChatId)
        Qt.callLater(function () { composer.forceActiveFocus() })
      }
    }
  }
  ...
  Connections {
    ...
    function onMessagesLoaded(chatId, chat, messages) {
      if (chatId !== root.activeChatId) return
    ...
    function onMessageArrived(chatId, message, chat) {
      if (chatId !== root.activeChatId) return
      ...
      if (root.opened && root.client && root.linked) root.client.markRead(chatId)
    }

    function onMessageStatusChanged(chatId, messageId, status) {
      root.patchMessage(messageId, { status: status })
    }

    function onMessageMedia(chatId, messageId, imagePath) {
      if (chatId !== root.activeChatId || !imagePath) return
  ...
  Process {
    ...
    // Quickshell's qmltypes omits QProcess::ExitStatus even though the runtime
    // signal supplies it, so qmllint cannot resolve the second parameter type.
    // qmllint disable signal-handler-parameters
    onExited: function (exitCode, exitStatus) {
      if (exitCode !== 0 && root.client) root.client.startLogin()
    }
  ...
  Timer {
    ...
    interval: 3000
    repeat: false
    onTriggered: if (root.client && root.activeChatId) root.client.setTyping(root.activeChatId, "paused")
  }
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      blocked: composer.activeFocus
        || apiIdField.activeFocus
        || apiHashField.activeFocus
        || passwordField.activeFocus
        || root.logoutConfirmOpen
        || root.peekActive
      ...
      Column {
        ...
        Item {
          ...
          Column {
            ...
            Text {
              ...
              width: parent.width
              text: root.view === "chat"
                ? Model.chatTitle(root.activeChat || { chatId: root.activeChatId, name: "" })
                : "OmaGram"
              color: root.foreground
              font.family: root.fontFamily
          ...
          Row {
            ...
            PanelActionButton {
              iconText: "\uf24d"
              tooltipText: "Open Telegram Desktop"
              foreground: root.foreground
              fontFamily: root.fontFamily
              onClicked: {
                root.openClient()
        ...
        Column {
          ...
          Column {
            width: parent.width
            spacing: Style.space(8)
            visible: root.needsApi

            Text {
              width: parent.width
              text: "Create an app at my.telegram.org \u2192 API development tools, then paste the values."
            ...
            Button {
              anchors.horizontalCenter: parent.horizontalCenter
              text: "Save"
              enabled: apiIdField.text.length > 0 && apiHashField.text.length === 32
              foreground: root.foreground
              fontFamily: root.fontFamily
              bordered: true
              onClicked: {
                if (!root.client) return
                root.client.setApi(apiIdField.text, apiHashField.text)
                apiIdField.text = ""
                apiHashField.text = ""
                root.client.startLogin()
              }
            }
          }
          ...
          Column {
            width: parent.width
            spacing: Style.space(8)
            visible: !root.needsApi && root.client
              && root.client.connectionState === "password"
            ...
            Text {
              width: parent.width
              textFormat: Text.PlainText
              visible: root.client && root.client.passwordHint.length > 0
              text: root.client ? root.client.passwordHint : ""
              color: root.secondaryForeground
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
              horizontalAlignment: Text.AlignHCenter
          ...
          Column {
            width: parent.width
            visible: !root.needsApi && root.client
              && root.client.connectionState === "code"

            Text {
              width: parent.width
              text: "Finish the phone login in the terminal running omarchy-omagram login --phone"
              color: root.secondaryForeground
              font.family: root.fontFamily
              font.pixelSize: Style.font.body
              horizontalAlignment: Text.AlignHCenter
          ...
          Column {
            ...
            Text {
              ...
              visible: root.hasQr
              horizontalAlignment: Text.AlignHCenter
              text: "Telegram \u2192 Settings \u2192 Devices \u2192 Link Desktop Device"
              color: root.secondaryForeground
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
              wrapMode: Text.WordWrap
            ...
            Button {
              ...
              bordered: true
              onClicked: {
                if (root.client && root.client.startLogin() === false)
          ...
          ListView {
            ...
            delegate: CursorSurface {
              ...
              MouseArea {
                ...
                onClicked: root.selectChat(chatRow.modelData.chatId)
              }
            }
          }
        }
        ...
        Column {
          ...
          Item {
            ...
            TextField {
              ...
              onTextChanged: {
                if (!root.client || !root.activeChatId || !text.length) return
                if (!typingTimer.running) root.client.setTyping(root.activeChatId, "composing")
                typingTimer.restart()
              }
      ...
      ConfirmDialog {
        ...
        z: 10
        focus: opened
        message: "Log out and end this Telegram session?"
        confirmText: "Log out"
        foreground: root.foreground
        fontFamily: root.fontFamily
  ...
  PanelWindow {
    ...
    WlrLayershell.namespace: "omarchy-omagram-peek"
